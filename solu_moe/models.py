...
def solu(
    ...
    if low_prob_dropout > 0.0 and training:
        mask = (torch.rand_like(probs) < low_prob_dropout) * (
            probs.detach() < low_prob_dropout_threshold
        )
        probs = probs * (~mask)

    if inverse_prob_dropout > 0.0 and training:
        mask = (probs < torch.rand_like(probs)) * (
            torch.rand_like(probs) < inverse_prob_dropout
        )
        probs = probs * (~mask)
...
def solu_on_last_dims(
    ...
    acts = solu(
        preacts.view(*preacts.shape[:-n_dims], -1),
...
class MoeLayer(nn.Module):
    def __init__(
        ...
        self.experts_w_in = nn.ModuleList(
            [nn.Linear(d_in, d_expert) for _ in range(n_experts)]
        )
        if self.w_in_bias_init is not False:
            for expert in self.experts_w_in:
                expert.bias.data.fill_(self.w_in_bias_init)
        self.experts_w_out = nn.ModuleList(
            [nn.Linear(d_expert, d_out, bias=False) for _ in range(n_experts)]
        )
        ...
        if activation in {"solu", "grouped_solu"}:
            ...
            self.activation = SoLU(
                jitter_eps=self.solu_jitter_eps,
                inverse_prob_dropout=self.inverse_prob_dropout,
                low_prob_dropout=self.low_prob_dropout,
                low_prob_dropout_threshold=self.low_prob_dropout_threshold,
                grouped=grouped,
    ...
    def get_routing_info(self, hidden_states):
        ...
        if self.n_experts == 2:
            ...
            k_expert_weights[:, 1][
                torch.rand_like(k_expert_weights[:, 1])
                < (1 - (2 * k_expert_weights[:, 1]))
            ] = 0.0

        expert_counts = k_expert_indices.flatten().bincount(minlength=self.n_experts)
        load_balancing_loss = (
            self.n_experts
            * ((expert_counts / hidden_states.shape[0]) * mean_expert_probs).sum()
        )
    ...
    def get_expert_batches(self, hidden_states, expert_indices, expert_counts):
        ...
        if len(hidden_states.shape) == 3:
            ...
            hidden_states = hidden_states.reshape(
                hidden_states.shape[0] * self.k_experts, -1
            )
        ...
        if hidden_states.shape[0] == expert_indices.shape[0] / self.k_experts:
            ...
            """
            hidden_states = hidden_states.repeat(repeats=(1, self.k_experts)).view(
    ...
    def undo_expert_batching(self, expert_outputs, expert_perm):
        ...
        og_ordered_exp_outputs = catted_expert_batches[invert_perm(expert_perm)].view(
            -1, self.k_experts, catted_expert_batches.shape[-1]
        )
    ...
    def forward(self, x, return_expert_acts=False):
        ...
        (
            ...
            expert_counts,
            load_balancing_loss,
        ) = self.get_routing_info(x)
        input_expert_batches, expert_perm = self.get_expert_batches(
            x, k_expert_indices, expert_counts
        )
        ...
        hidden_state_expert_batches, expert_perm = self.get_expert_batches(
            acts, k_expert_indices, expert_counts
        )
    ...
    def get_full_acts_for_rendering(
        self, x, value_weighted=False, col_weight=-0.1, null_expert_activation=0.0
    ):
        """
        ...
        if value_weighted is True:
            neuron_writeout_norms = torch.stack(
                [expert.weight.data.norm(dim=0) for expert in self.experts_w_out], dim=0
            )
            acts = acts * neuron_writeout_norms[k_expert_indices]

        full_acts = (
            torch.ones(acts.shape[0], self.n_experts, self.d_expert)
            * null_expert_activation
        )
        for i in range(k_expert_indices.shape[0]):
            full_acts[i, k_expert_indices[i]] = acts[i]

        list_of_expert_acts = list(
            rearrange(full_acts, "batch expert neurons -> expert batch neurons")
        )
        separating_column = torch.ones(acts.shape[0], 1) * col_weight

        full_acts = torch.cat(
            [
                torch.cat([expert_acts, separating_column], dim=1)
                for expert_acts in list_of_expert_acts[:-1]
            ]
            + [list_of_expert_acts[-1]],
            dim=1,
        )
...
class MoeAutoencoder(nn.Module):
    ...
    def w_ins(self):
        weights = torch.stack(
            [expert.weight for expert in self.moe_layer.experts_w_in], dim=0
        )
        biases = torch.stack(
            [expert.bias for expert in self.moe_layer.experts_w_in], dim=0
        )
    ...
    def w_outs(self):
        weights = torch.stack(
            [expert.weight for expert in list(self.moe_layer.experts_w_out)], dim=0
        )
    ...
    def get_full_acts_for_rendering(
        self, x, value_weighted=False, col_weight=-0.05, null_expert_activation=0.0
    ):
        x = self.encode(x)
        (
            full_acts,
            k_expert_indices,
            k_expert_weights,
        ) = self.moe_layer.get_full_acts_for_rendering(
            x,
            value_weighted=value_weighted,
            col_weight=col_weight,
            null_expert_activation=null_expert_activation,
        )
        return full_acts, k_expert_indices, k_expert_weights
